...
// Convert the internal ranking signals into a friendly 0-100 "matchmaking score"
// the wingman can understand. Driven by the parts that reflect owner<>candidate
// fit (shared context + personality), mapped into an encouraging 62-98 range.
function compatibilityPercent(scoreParts) {
  const sharedContext = scoreParts?.sharedContext ?? 0;
  const personality = scoreParts?.personality ?? 0;
  const reciprocity = scoreParts?.reciprocity ?? 0;
  const raw = Math.min(1, sharedContext * 0.4 + reciprocity * 0.3 + personality * 0.3);
  return Math.round(62 + raw * 36);
}
...
function serializeCandidate(candidate, score, explainability) {
  ...
  return {
    ...
    photos: sortedPhotos.map((p) => (p.url || (p.filename ? `/uploads/${uid}/${p.filename}` : null))).filter(Boolean),
    prompts: sortedPhotos.filter((p) => p.prompt).map((p) => ({ prompt: p.prompt, prompt_answer: p.prompt_answer })),
    ranking: {
      score: Number(score.toFixed(4)),
      compatibility: compatibilityPercent(explainability?.scores),
      explainability,
    },
  };
}
...
// How well the OWNER fits what the CANDIDATE wants (soft, two-way fit).
// Surfaces people likely to swipe back — the product needs a *mutual* yes.
function reciprocityScore(owner, candidate) {
  ...
  if (typeof filters.age_min === 'number' || typeof filters.age_max === 'number') {
    total += 1;
    const age = typeof owner.age === 'number' ? owner.age : null;
    const min = typeof filters.age_min === 'number' ? filters.age_min : 0;
    const max = typeof filters.age_max === 'number' ? filters.age_max : 200;
    if (age === null || (age >= min && age <= max)) hits += 1;
  }

  const schoolPref = normalizeString(filters.school);
  if (schoolPref) {
    total += 1;
    if (schoolPref === normalizeString(owner.school)) { hits += 1; reasons.push('their type'); }
  }

  const majorPref = normalizeArray(filters.majors);
  if (majorPref.length) {
    total += 1;
    if (majorPref.some((major) => getCandidateMajors(owner).includes(major))) hits += 1;
  }

  const yearPref = normalizeString(filters.year);
  if (yearPref) {
    total += 1;
    if (yearPref === normalizeString(owner.year)) hits += 1;
  }

  if (total === 0) return { score: 0.6, reasons: [] };
  return { score: hits / total, reasons };
}

// Greedy diversity (MMR-lite): rank by score but down-weight candidates that
// share an exact school+major+year signature with ones already picked, so the
// deck isn't ten identical profiles in a row.
function profileSignature(candidate) {
  return [normalizeString(candidate.school), getCandidateMajors(candidate)[0] || '', normalizeString(candidate.year)].join('|');
}

function selectDiverse(ranked, limit) {
  const pool = ranked.slice().sort((a, b) => b.finalScore - a.finalScore);
  const selected = [];
  ...
  while (selected.length < limit && pool.length) {
    ...
    for (let i = 0; i < pool.length; i += 1) {
      const seen = seenSig.get(profileSignature(pool[i].candidate)) || 0;
      const penalty = 1 - Math.min(0.5, seen * 0.18);
      const adjusted = pool[i].finalScore * penalty;
    ...
    const sig = profileSignature(picked.candidate);
...
export async function getRankedCandidates({ ownerId, delegateId, limit = 25, randomness = 0.15 }) {
  ...
  for (const candidate of candidatePool) {
    ...
    const reciprocity = reciprocityScore(ownerNormalized, candidateNormalized);
    ...
    const scoreParts = {
      ...
      reciprocity: Number(reciprocity.score.toFixed(4)),
    ...
    const reasons = [...context.reasons, ...reciprocity.reasons];
    if (reciprocity.score >= 0.75) reasons.push('likely to like back');
    if (personality > 0.5) reasons.push('similar vibe');
  ...
  const limitN = Math.max(1, Math.min(100, Number(limit) || 25));
  const limited = selectDiverse(ranked, limitN);
