...
function isGenderPreferred(lookingFor, gender) {
  ...
  if (!normalizedLookingFor || normalizedLookingFor.includes('everyone')) return true;
  if (normalizedLookingFor.includes('men')) return normalizedGender === 'man';
  if (normalizedLookingFor.includes('women')) return normalizedGender === 'woman';
  if (normalizedLookingFor.includes('non-binary')) return normalizedGender === 'non-binary';
  return true;
}
...
function passesHardConstraints(owner, candidate, filters) {
  ...
  if (filters.age_min && typeof candidate.age === 'number' && candidate.age < filters.age_min) {
    return { pass: false, reason: 'age minimum' };
  }
  if (filters.age_max && typeof candidate.age === 'number' && candidate.age > filters.age_max) {
  ...
  if (distanceMiles > distanceLimit) {
...
export async function getRankedCandidates({ ownerId, delegateId, limit = 25, randomness = 0.15 }) {
  ...
  const swipedRows = await Swipe.find({ owner_user_id: ownerId }).select('target_user_id').lean();
  const swipedIds = swipedRows.map((row) => row.target_user_id.toString());

  const excludeIds = [...new Set([ownerId, delegateId, ...swipedIds])];
