...
const PhotoSchema = new mongoose.Schema({
  ...
  // `url` is the canonical location for new uploads (Vercel Blob, a full https URL).
  // `filename` is legacy — older photos served from public/uploads/{userId}/{filename}.
  // A photo has one or the other; the resolver in lib/photos.js prefers `url`.
  url: { type: String, default: null },
  blob_pathname: { type: String, default: null }, // blob key, for deletion
...
const UserSchema = new mongoose.Schema({
  ...
  email_verification_code_hash: { type: String, default: null },
  ...
  hidden_prompt:      { type: String, default: null },
  hidden_prompt_answer: { type: String, default: null },
  ...
  pro_since:          { type: Date, default: null },
  ...
  token_version:      { type: Number, default: 0 },  // bump to invalidate all existing JWTs
